...
public class ConstantCapProvider implements ICapabilityProvider {
    LightColor color = null;

    public ConstantCapProvider(LightColor c) {
        this.color = c;
    }
    ...
    public <T> T getCapability(@Nonnull Capability<T> capability, @Nullable EnumFacing facing) {
        if(capability == LightCapabilityHandler.LIGHT_PROVIDER_CAPABILITY)
            return (T) new ConstantLightProvider();
        else
            return null;
    }

    private class ConstantLightProvider implements ILightProvider {
